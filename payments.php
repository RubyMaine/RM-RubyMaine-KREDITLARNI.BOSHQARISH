		...
		<div class="card">
			<div class="card-header">
				<large class="card-title">
					<b>Список платежей</b>
					<button class="btn btn-primary btn-sm btn-block col-md-2 float-right" type="button" id="new_payments"><i class="fa fa-plus"></i> Добавить новый платеж: </button>
				</large>
				...
				<table class="table table-bordered" id="loan-list">
					...
					<thead>
						<tr>
							...
							<th class="text-center">Номер кредита:</th>
							<th class="text-center">Получатель платежа:</th>
...
<script>
	...
	$('#new_payments').click(function(){
		uni_modal("Новый платеж","manage_payment.php",'mid-large')
	})
	$('.edit_payment').click(function(){
		uni_modal("Изменить платеж","manage_payment.php?id="+$(this).attr('data-id'),'mid-large')
	})
	$('.delete_payment').click(function(){
		_conf("Вы уверены, что хотите удалить эти данные?","delete_payment",[$(this).attr('data-id')])
	})
function delete_payment($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Платеж успешно удален",'Успех')
					setTimeout(function(){
						location.reload()
					},1500)
